...
import { Sparkles, ExternalLink, Megaphone, ArrowLeft, ArrowRight, Zap } from 'lucide-react';
...
export const SponsoredBanner: React.FC<SponsoredBannerProps> = ({
  ...
  if (ad.placement === 'in_article_top') {
    return (
      <div 
        dir={isAr ? 'rtl' : 'ltr'}
        className={`my-3 p-3.5 sm:p-4 rounded-2xl border transition-all ${
          isDarkMode
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-indigo-600 text-white">
              {ad.badgeText || (isAr ? 'إعلان مميز' : 'Sponsored')}
            </span>
            <div>
          ...
          >
            <span>{ad.ctaText || (isAr ? 'اكتشف المزيد' : 'Learn More')}</span>
    ...
    return (
      ...
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-500 text-white">
                  {ad.badgeText || (isAr ? 'عرض حصري' : 'Exclusive Offer')}
                </span>
                <h5 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white">
                  {ad.title}
                </h5>
          ...
          >
            <span>{ad.ctaText || (isAr ? 'الحصول على العرض' : 'Claim Offer')}</span>
            ...
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-bold bg-indigo-600 text-white shadow-xs">
                  ...
                  {ad.badgeText || (isAr ? 'إعلان برعاية' : 'Sponsored')}
                </span>
    ...
    return (
      ...
      >
        <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-amber-500 text-white mb-3 shadow-xs">
          ...
          {ad.badgeText || (isAr ? 'إعلان معتمد' : 'Verified Partner')}
        </span>
        ...
        >
          <span>{ad.ctaText || (isAr ? 'اكتشف الآن' : 'Explore Now')}</span>
    ...
    >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800">
        <span className="text-xs font-bold text-amber-600 dark:text-amber-400 flex items-center gap-1">
          {ad.ctaText || (isAr ? 'زيارة العرض' : 'Visit Deal')}
          <ArrowIcon className="w-3.5 h-3.5 group-hover:-translate-x-1 transition-transform" />
        </span>
        <span className="text-[10px] text-slate-400">{isAr ? 'نقرات:' : 'Clicks:'} {ad.clicks || 0}</span>
      </div>
    </div>
  );
};
