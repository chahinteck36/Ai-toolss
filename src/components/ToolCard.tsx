...
const ToolCardComponent: React.FC<ToolCardProps> = ({
  ...
  const getPricingBadge = (pricing: PricingType, pricingAr: string) => {
    ...
    if (!isAr) {
      switch (pricing) {
        case 'free': label = '100% Free'; break;
        case 'freemium': label = 'Freemium'; break;
        case 'open_source': label = 'Open Source'; break;
        case 'free_trial': label = 'Free Trial'; break;
        case 'paid': label = 'Paid'; break;
        default: label = 'Available'; break;
      }
    }
  ...
  const toolTitle = isAr ? tool.nameAr : tool.nameEn;
  const toolSubtitle = isAr ? `(${tool.nameEn})` : `(${tool.nameAr})`;
  const toolTagline = isAr ? tool.taglineAr : (tool.taglineEn || tool.descriptionEn || tool.taglineAr);
  const toolDesc = isAr ? tool.descriptionAr : (tool.descriptionEn || tool.descriptionAr);
      ...
      >
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {tool.alternativeTo && tool.alternativeTo.length > 0 && (
                <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-50 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                  ...
                  {isAr ? `بديل لـ: ${tool.alternativeTo[0]}` : `Alt to: ${tool.alternativeTo[0]}`}
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0 self-end sm:self-center">
          ...
          <a
            ...
            className="px-3 py-1.5 text-xs font-semibold rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 transition-colors inline-flex items-center justify-center cursor-pointer"
          >
  ...
  return (
    <div 
      ...
      dir={isAr ? 'rtl' : 'ltr'}
      className={`group relative rounded-2xl border transition-all duration-200 ease-out flex flex-col justify-between hover:-translate-y-1 hover:shadow-xl ${
        isDarkMode 
          ? 'bg-slate-900 border-slate-800 hover:border-indigo-500/50 hover:shadow-indigo-950/30 shadow-slate-950/50' 
          : 'bg-white border-slate-200/90 hover:border-indigo-400/80 hover:shadow-indigo-500/10 shadow-xs shadow-slate-200/40'
      }`}
    >
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {tool.isFeatured && (
                  <span className="flex items-center text-amber-500 group-hover:rotate-12 transition-transform duration-300" title={isAr ? 'أداة مميزة' : 'Featured'}>
                    <Sparkles className="w-3.5 h-3.5 fill-amber-400" />
                  </span>
                )}
                {tool.isPopular && (
                  <span className="flex items-center text-rose-500 group-hover:scale-110 transition-transform duration-300" title={isAr ? 'شعبية عالية' : 'Popular'}>
                    <Flame className="w-3.5 h-3.5 fill-rose-500" />
                  </span>
                )}
              </div>
              <p className="text-xs font-medium text-slate-400 truncate">
                {isAr ? tool.nameEn : tool.nameAr}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              title={copied ? (isAr ? 'تم نسخ الرابط!' : 'Copied!') : (isAr ? 'مشاركة رابط الأداة' : 'Share tool link')}
            >
            ...
            <button
              ...
              className={`p-2 rounded-xl transition-transform active:scale-90 cursor-pointer ${
                isFavorite 
        ...
        <div className="flex items-center gap-1.5 mb-2.5 flex-wrap">
          ...
          {tool.category === 'academic_scholar' && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200 border border-emerald-300 dark:border-emerald-700">
              ...
              {isAr ? 'حقيبة أكاديمية' : 'Scholar Pack'}
            </span>
          )}
          ...
          {tool.alternativeTo && tool.alternativeTo.length > 0 && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold bg-purple-100 text-purple-800 dark:bg-purple-950/70 dark:text-purple-300 border border-purple-200 dark:border-purple-800 truncate max-w-[180px]">
              ...
              <span>{isAr ? `بديل ${tool.alternativeTo[0]}` : `Alt to ${tool.alternativeTo[0]}`}</span>
            </span>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {tool.tags.slice(0, 3).map((tag) => (
            <button
              ...
              className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium bg-slate-100 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 transition-colors cursor-pointer"
            >
      ...
      <div className={`p-4 pt-3 border-t rounded-b-2xl transition-colors ${
        ...
        <div className="flex items-center justify-between mb-3 text-xs">
          <div className="flex items-center gap-1">
            <div className="flex items-center gap-0.5" title={isAr ? 'انقر لتقييم الأداة' : 'Rate this tool'}>
              {[1, 2, 3, 4, 5].map((starIndex) => {
                ...
                return (
                  <button
                    ...
                    title={isAr ? `تقييم ${starIndex} من 5` : `Rate ${starIndex} of 5`}
                  >
                    <Star 
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-1.5 py-2 px-3 text-xs font-bold rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white active:scale-95 transition-all shadow-xs shadow-indigo-500/20 hover:shadow-indigo-500/30 cursor-pointer"
          >
            <span>{isAr ? 'زيارة الأداة' : 'Visit Site'}</span>
